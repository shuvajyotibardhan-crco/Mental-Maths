...
interface ScienceResultsScreenProps {
  gameState: ScienceGameState
  onPlayAgain: () => void
  onNavigate: (screen: string) => void
}

export function ScienceResultsScreen({
  gameState,
  onPlayAgain,
  onNavigate,
}: ScienceResultsScreenProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 gap-6 min-h-[70vh]">
      <div className="text-center animate-bounce-in">
        ...
        <p className="text-gray-500 text-sm mt-1">Science Quiz Complete</p>
      </div>
            ...
            {answered.filter((a) => !a.isCorrect).map((a, i) => (
              <div key={i} className="bg-red-50 rounded-2xl px-4 py-3 text-sm">
                ...
                <div className="space-y-0.5">
                  {a.question.correctIndices.map((ci) => (
                    <p key={ci} className="text-emerald-700">
                      ✓ {a.question.options[ci]}
                    </p>
                  ))}
                </div>
              </div>
