...
All Firebase Authentication logic. Uses a **synthetic email system**: since Firebase Auth requires an email, usernames are mapped to `username@mentalmaths.app`. The recovery email is stored in Firestore only — the Firebase Auth account always uses the synthetic email. Self-service password reset is handled by the `sendPasswordResetLink` Cloud Function (not client-side). Login tries the synthetic email first and falls back to the recovery email for legacy accounts. Key functions: `registerUser`, `loginUser`, `logoutUser`, `changePassword`, `updateRecoveryEmail`, `resetPasswordByUsername`.
...
### `functions/index.js`
Firebase Cloud Functions (Node 22, v2). Exports three `onCall` functions: `sendPasswordResetLink` (unauthenticated — looks up user by recovery email in Firestore, generates a reset link via Admin SDK for the synthetic email, and sends it to the recovery address via nodemailer/Gmail SMTP); `adminSetPassword` (admin-only — sets target user's password via `auth().updateUser()`); `adminDeleteUser` (admin-only — deletes target user's Auth account). Both admin functions verify the caller's UID is in `admins/{uid}`. Deployed via `firebase-tools` in CI.
...
Firebase Authentication requires an email address. Rather than making email mandatory (which excludes young users who may not have one), a synthetic email (`username@mentalmaths.app`) is generated internally. Users never see it. A real recovery email can be optionally added for password reset — it is stored in Firestore `usernameLookup` only; the Firebase Auth account always retains the synthetic email. The `sendPasswordResetLink` Cloud Function generates a reset link for the synthetic email and delivers it to the recovery address via nodemailer.
