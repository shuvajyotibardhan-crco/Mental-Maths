...
Custom hook encapsulating all Word-O-Meter solo game logic. `startGame(grade, letterCount)` picks a word and initialises state. `typeLetter` / `deleteLetter` manage the current-row buffer. `submitGuess` evaluates the guess letter-by-letter against the target (two-pass algorithm: correct positions first, then present letters, to handle duplicate letters correctly), updates `guesses` and `letterStates`, and transitions to `won`/`lost` when appropriate. `useHint(type)` consumes an available hint and generates its text. `forceFinish` ends the session early. `_finishSession` calls `saveWordOMeterSession` and `saveSeenWordToStorage` asynchronously. Score: `max(10, 100 − (attemptsUsed−1) × 12 − hintsUsed × 8)` on win; 0 on loss.
